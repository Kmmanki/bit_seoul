...
def build_model(drop=0.5, optimizer=Adam, lr = 0.001
    ...
    x = Dense(512,  activation=activation, name='hidden1')(input)
    x = Dropout(drop)(x)

    for i in range(layernum):
        x = Dense(nodenum1, activation=activation, name='nodenum1_'+str(i))(x)
        x = Dropout(drop)(x)
        x = Dense(nodenum2, activation=activation, name='nodenum2_'+str(i))(x)
        x = Dropout(drop)(x)
